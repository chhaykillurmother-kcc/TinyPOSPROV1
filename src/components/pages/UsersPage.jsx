...
function safeConfirm(promptMessage) {
  try {
  ...
  async function removeCustomRole(role) {
    if (!safeConfirm(`Delete ${role.name}? Existing staff must be moved to another role first.`)) return;
    try {
      setBusy(true);
        ...
        <>
          ...
          {loading ? (
            ...
            <ResponsiveDataList
              ...
              columns={[
                { label: t("Staff"), width: 190, value: (member) => member.full_name || "—", render: (member) => <div className="user-primary-cell"><strong>{member.full_name || "—"}</strong>{member.id === profile.id && <small>{t("You")}</small>}</div> },
                ...
                { label: t("Branch"), width: 170, value: (member) => member.branches?.name || t("No branch"), render: (member) => <div className="user-primary-cell"><strong>{member.branches?.name || t("No branch")}</strong><small>{member.branches?.code || "—"}</small></div> },
                ...
                { label: t("Actions"), actionsOnly: true, excludeDocument: true, render: (member) => <div className="staff-table-actions"><button type="button" className="secondary-button compact-button" disabled={!canEdit(member)} onClick={() => { setEditingStaff(member); setStaffFormOpen(true); }}><Edit3 size={16} /><span>{t("Edit")}</span></button><button type="button" className="secondary-button compact-button" disabled={!canEdit(member)} onClick={() => setResetMember(member)}><KeyRound size={16} /><span>{t("Password")}</span></button>{canChangeStatus(member) && <button type="button" className={member.is_active ? "danger-text-button" : "success-text-button"} disabled={busy} onClick={() => toggleStaff(member)}><span>{member.is_active ? t("Deactivate") : t("Activate")}</span></button>}</div> }
              ]}
              renderCard={(member) => (
                <article className="responsive-data-card staff-directory-card">
                  ...
                  <footer>
                    <button type="button" className="secondary-button compact-button" disabled={!canEdit(member)} onClick={() => { setEditingStaff(member); setStaffFormOpen(true); }}><Edit3 size={16} /><span>{t("Edit")}</span></button>
                    <button type="button" className="secondary-button compact-button" disabled={!canEdit(member)} onClick={() => setResetMember(member)}><KeyRound size={16} /><span>{t("Password")}</span></button>
                    {canChangeStatus(member) && <button type="button" className={member.is_active ? "danger-text-button" : "success-text-button"} disabled={busy} onClick={() => toggleStaff(member)}><span>{member.is_active ? t("Deactivate") : t("Activate")}</span></button>}
                  </footer>
                </article>
      ...
      {tab === "branches" && (
        loading ? (
          ...
          <ResponsiveDataList
            ...
            headingExtra={<button type="button" className="primary-button" onClick={() => { setEditingBranch(null); setBranchFormOpen(true); }}><Plus size={18} /><span>{t("Add branch")}</span></button>}
            columns={[
              { label: t("Branch"), width: 180, value: (branch) => branch.name || "—", render: (branch) => <div className="user-primary-cell"><strong>{branch.name || "—"}</strong><small>{branch.code || "—"}</small></div> },
              ...
              { label: t("Actions"), actionsOnly: true, excludeDocument: true, render: (branch) => <div className="branch-table-actions"><button type="button" className="secondary-button compact-button" onClick={() => { setEditingBranch(branch); setBranchFormOpen(true); }}><Edit3 size={16} /><span>{t("Edit")}</span></button><button type="button" className={branch.is_active ? "danger-text-button" : "success-text-button"} disabled={busy} onClick={() => toggleBranch(branch)}><span>{branch.is_active ? t("Deactivate") : t("Activate")}</span></button></div> }
            ]}
            renderCard={(branch) => (
      ...
      {tab === "roles" && (
        loading ? (
          ...
          <ResponsiveDataList
            ...
            headingExtra={<button type="button" className="primary-button" onClick={() => { setEditingCustomRole(null); setCustomRoleOpen(true); }}><Plus size={18} /><span>{t("Add custom role")}</span></button>}
            columns={[
              { label: t("Role"), width: 180, value: (role) => role.name || "—", render: (role) => <div className="user-primary-cell"><strong>{t(role.name) || "—"}</strong><small>{role.kind === "standard" ? t("Standard role") : t("Custom role")}</small></div> },
              { label: t("Based on"), width: 125, value: (role) => t(roleLabel(role.base_role)) },
              { label: t("Description"), width: 320, value: (role) => role.description ? t(role.description) : t("No description."), render: (role) => <div className="role-table-description">{role.description ? t(role.description) : t("No description.")}</div> },
              ...
              { label: t("Actions"), actionsOnly: true, excludeDocument: true, render: (role) => role.kind === "custom" ? <div className="role-table-actions"><button type="button" className="secondary-button compact-button" onClick={() => { setEditingCustomRole(role); setCustomRoleOpen(true); }}><Edit3 size={16} /><span>{t("Edit")}</span></button><button type="button" className="danger-text-button" disabled={busy || Number(role.assigned_staff_count || 0) > 0} onClick={() => removeCustomRole(role)}><span>{t("Delete")}</span></button></div> : <span className="muted">{t("System role")}</span> }
            ]}
            renderCard={(role) => (
              <article className="responsive-data-card role-directory-card">
                <header>
                  <div className="role-directory-card-title"><span className="role-icon"><ShieldCheck size={20} /></span><span><strong>{t(role.name) || "—"}</strong><small>{role.kind === "standard" ? t("Standard role") : `${t("Based on")} ${t(roleLabel(role.base_role))}`}</small></span></div>
                  <span className={`status-pill ${role.is_active ? "active" : "inactive"}`}>{role.is_active ? t("Active") : t("Inactive")}</span>
                </header>
                <p className="role-directory-description">{role.description ? t(role.description) : t("No description.")}</p>
                ...
                <footer>{role.kind === "custom" ? <><button type="button" className="secondary-button compact-button" onClick={() => { setEditingCustomRole(role); setCustomRoleOpen(true); }}><Edit3 size={16} /><span>{t("Edit")}</span></button><button type="button" className="danger-text-button" disabled={busy || Number(role.assigned_staff_count || 0) > 0} onClick={() => removeCustomRole(role)}><span>{t("Delete")}</span></button></> : <span className="muted">{t("System role")}</span>}</footer>
              </article>
            )}
          />
        )
